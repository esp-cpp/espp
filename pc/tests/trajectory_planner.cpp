...
struct Stats {
  float max_v{0}, max_w{0}, max_a_v{0}, max_a_w{0};
  float prev_v{0}, prev_w{0}, prev_a_v{0}, prev_a_w{0};
  float prev_t{0};
  bool first{true};
  ...
  void update(const espp::TrajectoryPlanner::MotionCommand &cmd, float t) {
    ...
    if (!first) {
      ...
      if (dt > 0) {
        ...
        max_a_v = std::max(max_a_v, a_v);
        max_a_w = std::max(max_a_w, a_w);
      }
    }
...
static bool test_trapezoidal(espp::Logger &log) {
  ...
  espp::TrajectoryPlanner planner({
      ...
      .output_callback =
          [&](const espp::TrajectoryPlanner::MotionCommand &cmd) {
            ...
            log.info("[{:.3f}] v={:+.3f}  w={:+.3f}", elapsed(), cmd.linear_velocity,
                     cmd.angular_velocity);
          },
      .planning_period = std::chrono::milliseconds(20),
      .callback_period = std::chrono::milliseconds(40),
  ...
  bool ok = (stats.max_v <= 1.0f * 1.05f) &&
            (stats.max_a_v <= 2.0f * 1.10f); // 10% tolerance for timing jitter
  log.info("  max|v|={:.3f}/1.0  max|a_v|={:.3f}/2.0  -> {}", stats.max_v, stats.max_a_v,
           ok ? "PASS" : "FAIL");
  return ok;
}
...
static bool test_scurve(espp::Logger &log) {
  ...
  bool ok = (stats.max_v <= 1.0f * 1.05f) && (stats.max_a_v <= 2.0f * 1.10f);
  log.info("  max|v|={:.3f}/1.0  max|a_v|={:.3f}/2.0  -> {}", stats.max_v, stats.max_a_v,
           ok ? "PASS" : "FAIL");
  return ok;
}
