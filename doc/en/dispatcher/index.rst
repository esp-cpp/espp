...
routes each frame to a per-module handler by the message-type byte's high-nibble
module id, so OTA, crash-dump inspection, a CAN bridge and an application's own
control channel can share one USB / socket / UART link without interfering.
