...
pool = espp.ThreadPool(espp.ThreadPool.Config(worker_count=2, auto_start=False))
check(not pool.is_running(),       "not running before start()")
...
pool.stop()
check(not pool.is_running(),       "not running after stop()")
...
s = pool.stats()
print(f"  stats: submitted={s.submitted} executed={s.executed} rejected={s.rejected}")
check(s.submitted == N, f"submitted == {N}")
check(s.executed == N,  f"executed == {N}")
...
# 3. try_submit() — deterministic rejection when queue is full
# ---------------------------------------------------------------------------
print("--- try_submit bounded queue ---")
barrier = threading.Event()
first_started = threading.Event()
...
pool.try_submit(_blocking_job)
first_started.wait(timeout=2.0)

# Fill the 2-slot queue
pool.try_submit(lambda: barrier.wait())
pool.try_submit(lambda: barrier.wait())

# Queue is now provably full — every further try_submit must be rejected
rejected = sum(1 for _ in range(3) if not pool.try_submit(lambda: None))
...
# ---------------------------------------------------------------------------
print("--- submit after stop ---")
pool = espp.ThreadPool(espp.ThreadPool.Config(worker_count=1))
pool.stop()
check(not pool.submit(lambda: None), "submit after stop() returns False")
check(pool.stats().rejected == 1,    "stats.rejected == 1")

# ---------------------------------------------------------------------------
# 5. Concurrent submit from multiple threads
# ---------------------------------------------------------------------------
print("--- concurrent submit ---")
NUM_THREADS = 4
JOBS_PER_THREAD = 10
...
pool = espp.ThreadPool(espp.ThreadPool.Config(worker_count=3))
...
def _producer():
    for _ in range(JOBS_PER_THREAD):
        def _work():
            ...
            with c_lock:
                c_counter[0] += 1
                if c_counter[0] >= TOTAL_JOBS:
                    c_done.set()
        pool.submit(_work)

threads = [threading.Thread(target=_producer) for _ in range(NUM_THREADS)]
for t in threads:
    t.start()
for t in threads:
    t.join()

c_done.wait(timeout=10.0)
s = pool.stats()
print(f"  stats: submitted={s.submitted} executed={s.executed} rejected={s.rejected}")
check(s.submitted == TOTAL_JOBS, "all concurrent submissions accepted")
check(s.executed == TOTAL_JOBS,  "all submitted jobs executed")
