...
bool M5StackCardputer::initialize_lcd() {
  if (lcd_ || backlight_) {
...
bool M5StackCardputer::initialize_display(size_t pixel_buffer_size) {
  if (!lcd_) {
    logger_.error(
...
std::shared_ptr<espp::Display<M5StackCardputer::Pixel>> M5StackCardputer::display() const {
  return display_;
}

M5StackCardputer::Pixel *M5StackCardputer::vram0() const {
  if (!display_) {
    return nullptr;
  }
...
M5StackCardputer::Pixel *M5StackCardputer::vram1() const {
  if (!display_) {
    return nullptr;
  }
...
void M5StackCardputer::brightness(float brightness) {
  if (!backlight_) {
    return;
  }
...
float M5StackCardputer::brightness() const {
  if (!backlight_) {
    return 0.0f;
  }
  auto maybe_duty = backlight_->get_duty(backlight_channel_configs_[0].channel);
  return maybe_duty.value_or(0.0f);
}

void IRAM_ATTR M5StackCardputer::lcd_wait_lines() {
  if (lcd_) {
    lcd_->wait();
  }
}

void M5StackCardputer::write_lcd_frame(const uint16_t xs, const uint16_t ys, const uint16_t width,
...
void IRAM_ATTR M5StackCardputer::write_lcd_lines(int xs, int ys, int xe, int ye,
