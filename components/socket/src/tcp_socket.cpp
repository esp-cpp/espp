...
bool TcpSocket::transmit(const std::vector<uint8_t> &data,
...
bool TcpSocket::transmit(const std::vector<char> &data,
                         const detail::TcpTransmitConfig &transmit_config) {
  return transmit(std::string_view{(const char *)data.data(), data.size()}, transmit_config);
}

bool TcpSocket::transmit(std::string_view data, const detail::TcpTransmitConfig &transmit_config) {
  if (!is_valid()) {
