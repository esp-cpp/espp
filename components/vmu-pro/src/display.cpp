...
bool VmuPro::initialize_display(size_t pixel_buffer_size) {
  if (!lcd_) {
    logger_.error(
...
std::shared_ptr<espp::Display<VmuPro::Pixel>> VmuPro::display() const { return display_; }

void IRAM_ATTR VmuPro::lcd_wait_lines() {
  if (lcd_) {
    lcd_->wait();
  }
}

void VmuPro::write_lcd_frame(const uint16_t xs, const uint16_t ys, const uint16_t width,
...
void IRAM_ATTR VmuPro::write_lcd_lines(int xs, int ys, int xe, int ye, const uint8_t *data,
                                       uint32_t user_data) {
  if (!lcd_) {
    return;
  }
...
VmuPro::Pixel *VmuPro::vram0() const {
  if (!display_) {
    return nullptr;
  }
...
VmuPro::Pixel *VmuPro::vram1() const {
  if (!display_) {
    return nullptr;
  }
...
size_t VmuPro::rotated_display_width() const {
...
size_t VmuPro::rotated_display_height() const {
  auto *display = lv_display_get_default();
  auto rotation = display ? lv_display_get_rotation(display) : LV_DISPLAY_ROTATION_0;
  switch (rotation) {
  case LV_DISPLAY_ROTATION_90:
