...
public:
   ...
   *       there the drain can never happen while this call blocks, so the
   *       write fails fast with `no_buffer_space` instead of stalling the USB
   *       stack. To reliably send frames larger than the TX FIFO in response
   *       to received data, queue the work to your own task rather than
   *       writing directly from the receive callback (or size the FIFO to
   *       hold a full frame).
   */
