...
namespace espp {
  ...
  public:
    ...
    struct Config {
      ...
      espp::Logger::Verbosity log_level{espp::Logger::Verbosity::WARN}; /**< Verbosity for the bldc driver. */
    };
     ...
     */
    float get_voltage_limit() const {
      return limit_voltage_.load();
    }
     ...
     */
    float get_power_supply_limit() const {
      return power_supply_voltage_.load();
    }
  ...
  protected:
    ...
    void configure_generator_action(mcpwm_gen_handle_t &gen_high, mcpwm_cmpr_handle_t comp) {
      ...
      ESP_ERROR_CHECK(mcpwm_generator_set_actions_on_compare_event(gen_high,
