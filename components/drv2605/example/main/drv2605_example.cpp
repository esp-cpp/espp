...
extern "C" void app_main(void) {
  ...
  // This example shows using the i2c haptic motor driver (drv2605)
  {
    ...
    auto i2c_write = [](uint8_t dev_addr, uint8_t* data, size_t data_len) {
      auto err = i2c_master_write_to_device(I2C_NUM,
                                            dev_addr,
                                            data,
                                            data_len,
    ...
    auto i2c_read = [](uint8_t dev_addr, uint8_t reg_addr, uint8_t* data, size_t data_len) {
      auto err = i2c_master_write_read_device(I2C_NUM,
                                              dev_addr,
                                              &reg_addr,
                                              1, // size of addr
                                              data,
    ...
    auto task_fn = [&drv2605](std::mutex& m, std::condition_variable& cv) {
      ...
      drv2605.set_waveform(0, (espp::Drv2605::Waveform)waveform);
      drv2605.set_waveform(1, espp::Drv2605::Waveform::END);
      drv2605.start();
      waveform++;
