...
void test_streaming_decoder(espp::Logger& logger) {
    ...
    // Test 1: Multiple packets with fragmented input
    {
        ...
        if (success) {
            ...
            logger.error("Test 1: FAIL - Multiple packets with fragmented input (extracted: {}, expected: 3)", packets_extracted);
        }
    }
...
void test_edge_cases(espp::Logger& logger) {
    ...
    // Test 3: Consecutive zeros
    {
        ...
        std::vector<uint8_t> encoded = Cobs::encode_packet(consecutive_zeros.data(), consecutive_zeros.size());
        std::vector<uint8_t> decoded = Cobs::decode_packet(encoded.data(), encoded.size());
        
        bool success = (decoded.size() == consecutive_zeros.size()) && 
    ...
    // Test 4: Large packet (1000 bytes)
    {
        ...
        if (success) {
            ...
            logger.error("Test 4: FAIL - Large packet (1000 bytes) (decoded: {}, expected: {}, encoded: {})", 
