...
size_t Cobs::encode_packet(const uint8_t* data, size_t length, uint8_t* output) {
    ...
    uint8_t* codep = encode++; // Output code pointer (points to first value of the block (where the code goes))
...
std::vector<uint8_t> Cobs::decode_packet(const uint8_t* encoded_data, size_t length) {
    ...
    const uint8_t* delimiter = static_cast<const uint8_t*>(std::memchr(encoded_data, DELIMITER, length));
    if (!delimiter) {
