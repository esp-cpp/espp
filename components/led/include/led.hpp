...
namespace espp {
  ...
  public:
    ...
    struct ChannelConfig {
      ...
      float duty{0}; /**< The starting duty cycle (%) [0, 100] that you want the LED channel to have. */
      ledc_mode_t speed_mode{LEDC_HIGH_SPEED_MODE}; /**< The LEDC speed mode you want for this LED channel. */
    ...
    Led(const Config& config)
      : duty_resolution_(config.duty_resolution),
        ...
        logger_({.tag = "Led", .level = config.log_level}) {
      ...
      ledc_cbs_t callbacks = {
        .fade_cb = &Led::cb_ledc_fade_end_event
      };
      ...
      for (int i=0; i<channels_.size(); i++) {
        ledc_cb_register(channels_[i].speed_mode,
                         channels_[i].channel,
                         &callbacks,
