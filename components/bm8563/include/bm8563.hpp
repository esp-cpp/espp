...
public:
  ...
  void set_date_time(DateTime &dt, std::error_code &ec) {
    set_date(dt.date, ec);
    if (ec) return;
    set_time(dt.time, ec);
  }
...
protected:
  ...
  void write_register(Registers reg, uint8_t* data, size_t size, std::error_code& ec) {
    uint8_t buf[size + 1];
    buf[0] = (uint8_t)reg;
...
[[maybe_unused]] static bool operator==(const espp::Bm8563::Date &lhs, const espp::Bm8563::Date &rhs) {
  return lhs.year == rhs.year && lhs.month == rhs.month && lhs.weekday == rhs.weekday &&
         lhs.day == rhs.day;
}

[[maybe_unused]] static bool operator==(const espp::Bm8563::Time &lhs, const espp::Bm8563::Time &rhs) {
  return lhs.hour == rhs.hour && lhs.minute == rhs.minute && lhs.second == rhs.second;
}

[[maybe_unused]] static bool operator==(const espp::Bm8563::DateTime &lhs, const espp::Bm8563::DateTime &rhs) {
...
template <> struct fmt::formatter<espp::Bm8563::Date> {
  ...
  auto format(espp::Bm8563::Date const &d, FormatContext &ctx) {
    return fmt::format_to(
        ctx.out(), "{{.year = {}, .month = {}, .weekday = {}, .day = {}}}", d.year, d.month,
        d.weekday, d.day);
  }
};
...
template <> struct fmt::formatter<espp::Bm8563::Time> {
  ...
  template <typename FormatContext>
  auto format(espp::Bm8563::Time const &t, FormatContext &ctx) {
    return fmt::format_to(ctx.out(), "{{.hour = {}, .minute = {}, .second = {}}}", t.hour,
                          t.minute, t.second);
  }
};
