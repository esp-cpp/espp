...
void Gui::init_background() {
  auto &round_display = espp::SsRoundDisplay::get();
  background_ = lv_obj_create(lv_screen_active());
  lv_obj_set_size(background_, round_display.rotated_display_width(),
...
void Gui::init_buttons() {
  ...
  // a button in the bottom middle which clears the circles
  clear_button_ = lv_btn_create(lv_screen_active());
  lv_obj_set_size(clear_button_, 50, 50);
  lv_obj_align(clear_button_, LV_ALIGN_BOTTOM_MID, 0, 0);
...
void Gui::init_circle_layer() {
  ...
  lv_obj_set_size(circle_layer_, round_display.rotated_display_width(),
                  round_display.rotated_display_height());
