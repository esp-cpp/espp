...
extern "C" void app_main(void) {
  ...
  // backpressure clear in `send` above.) These run on the TinyUSB task;
  // request_reset() defers the parser reset to the worker (the parser is only
  // ever touched there), while set_streaming is atomic and vendor_write_clear
  // touches only the TX FIFO, so both are fine to call here.
  usb.set_unmount_callback([&] {
    telemetry.set_streaming(false);
    usb.vendor_write_clear();
    usb_link.request_reset();
  });
