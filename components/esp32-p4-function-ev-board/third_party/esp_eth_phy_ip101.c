...
static esp_err_t ip101_page_select(phy_ip101_t *ip101, uint32_t page) {
  ...
  pcr_reg_t pcr = {.register_page_select = page};
  ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, ip101->phy_802_3.addr, ETH_PHY_PCR_REG_ADDR, pcr.val),
                    err, TAG, "write PCR failed");
  return ESP_OK;
err:
  return ret;
}
...
static esp_err_t ip101_update_link_duplex_speed(phy_ip101_t *ip101) {
  ...
  ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, addr, ETH_PHY_CSSR_REG_ADDR, &(cssr.val)), err, TAG,
                    "read CSSR failed");
  ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, addr, ETH_PHY_ANLPAR_REG_ADDR, &(anlpar.val)), err, TAG,
                    "read ANLPAR failed");
  ...
  if (ip101->phy_802_3.link_status != link) {
    ...
    if (link == ETH_LINK_UP) {
      ...
      ESP_GOTO_ON_ERROR(eth->on_state_changed(eth, ETH_STATE_SPEED, (void *)speed), err, TAG,
                        "change speed failed");
      ESP_GOTO_ON_ERROR(eth->on_state_changed(eth, ETH_STATE_DUPLEX, (void *)duplex), err, TAG,
    ...
    ESP_GOTO_ON_ERROR(eth->on_state_changed(eth, ETH_STATE_LINK, (void *)link), err, TAG,
                      "change link failed");
...
static esp_err_t ip101_get_link(esp_eth_phy_t *phy) {
  ...
  ESP_GOTO_ON_ERROR(ip101_update_link_duplex_speed(ip101), err, TAG,
