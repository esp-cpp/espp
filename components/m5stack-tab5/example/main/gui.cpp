...
void Gui::init_background() {
  ...
  lv_obj_set_size(background_, tab5.display_width(), tab5.display_height());
...
void Gui::init_gravity_lines() {
  ...
  kalman_line_points_[1] = {tab5.display_width(), tab5.display_height()};
  ...
  madgwick_line_points_[1] = {tab5.display_width(), tab5.display_height()};
...
void Gui::init_buttons() {
  ...
  // a button next to it which cycles the backlight brightness through
  // 25/50/75/100%
  brightness_button_ = lv_btn_create(lv_screen_active());
  lv_obj_set_size(brightness_button_, 50, 50);
  lv_obj_align(brightness_button_, LV_ALIGN_TOP_LEFT, 60, 0);
  lv_obj_t *brightness_label = lv_label_create(brightness_button_);
  lv_label_set_text(brightness_label, LV_SYMBOL_EYE_OPEN);
  lv_obj_align(brightness_label, LV_ALIGN_CENTER, 0, 0);
  lv_obj_add_event_cb(brightness_button_, event_callback, LV_EVENT_PRESSED, this);
...
void Gui::init_circle_layer() {
  ...
  lv_obj_set_size(circle_layer_, tab5.display_width(), tab5.display_height());
...
void Gui::next_rotation() {
  ...
  lv_obj_set_size(background_, tab5.rotated_display_width(), tab5.rotated_display_height());
  lv_obj_set_size(circle_layer_, tab5.rotated_display_width(), tab5.rotated_display_height());
  lv_obj_align(circle_layer_, LV_ALIGN_CENTER, 0, 0);
