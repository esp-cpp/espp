...
Esp32P4Wifi6PoeEth::Esp32P4Wifi6PoeEth()
    : BaseComponent("Esp32P4Wifi6PoeEth") {}

bool Esp32P4Wifi6PoeEth::initialize_ethernet() { return initialize_ethernet(EthernetConfig{}); }

bool Esp32P4Wifi6PoeEth::initialize_ethernet(const EthernetConfig &config) {
  if (ethernet_ && ethernet_->is_initialized()) {
    logger_.warn("Ethernet already initialized; the provided configuration is IGNORED "
                 "(reconfiguration is not supported). Returning true since the interface "
                 "is up and running with its original configuration.");
    return true;
  }
  ...
  // The board-specific part: the ESP32-P4 EMAC RMII pins + the IP101GRI PHY.
  // The PHY generates the 50 MHz REF_CLK from its 25 MHz crystal, which the
  // P4 receives on GPIO50 (external clock input).
