...
bool Esp32P4Nano::initialize_lcd() {
  ...
  // channel 3 @ 2500 mV on the ESP32-P4-NANO).
  ...
  // NOTE: The ESP32-P4-NANO does not route a panel reset GPIO for either panel;
  ...
  // NOTE: The ESP32-P4-NANO has no backlight GPIO. The backlight is driven by an
...
void Esp32P4Nano::apply_panel_params(DisplayController controller) {
  display_controller_ =
...
bool Esp32P4Nano::initialize_display(size_t pixel_buffer_size) {
  ...
  if (!display_) {
    display_ = std::make_shared<Display<Pixel>>(
        Display<Pixel>::LvglConfig{.width = display_width_,
                                   ...
                                   .flush_callback = std::bind_front(&Esp32P4Nano::flush, this),
...
size_t Esp32P4Nano::rotated_display_width() const {
...
size_t Esp32P4Nano::rotated_display_height() const {
  auto rot = lv_display_get_rotation(lv_display_get_default());
  switch (rot) {
  case LV_DISPLAY_ROTATION_90:
  case LV_DISPLAY_ROTATION_270:
...
void Esp32P4Nano::write_lcd_lines(int xs, int ys, int xe, int ye, const uint8_t *data,
                                  uint32_t user_data) {
  (void)user_data;
...
void Esp32P4Nano::brightness(float brightness) {
  // The ESP32-P4-NANO has NO backlight GPIO. The backlight is driven by an
  // on-board I2C controller (addr ~0x45/0x95, chip/protocol not yet identified);
  // brightness control is a TODO pending hardware identification. The panel powers
...
float Esp32P4Nano::brightness() const { return brightness_.load(); }

void IRAM_ATTR Esp32P4Nano::flush(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map) {
  if (lcd_handles_.panel == nullptr) {
...
bool IRAM_ATTR Esp32P4Nano::notify_lvgl_flush_ready(esp_lcd_panel_handle_t panel,
                                                    esp_lcd_dpi_panel_event_data_t *edata,
                                                    void *user_ctx) {
  (void)panel;
  (void)edata;
  auto *board = static_cast<espp::Esp32P4Nano *>(user_ctx);
...
void Esp32P4Nano::dsi_write_command(uint8_t cmd, std::span<const uint8_t> params,
                                    uint32_t /*flags*/) {
  if (!lcd_handles_.io) {
...
void Esp32P4Nano::dsi_read_command(uint8_t cmd, std::span<uint8_t> data, uint32_t /*flags*/) {
  if (!lcd_handles_.io) {
