...
bool Esp32P4Nano::initialize_audio(uint32_t sample_rate,
  ...
  audio_task_ = espp::Task::make_unique(
      {.callback = std::bind(&Esp32P4Nano::audio_task_callback, this, _1, _2, _3),
...
void Esp32P4Nano::set_speaker_enabled(bool enable) {
  gpio_set_level(audio_pa_enable_io, enable ? 1 : 0);
}

void Esp32P4Nano::volume(float volume) {
  volume = std::clamp(volume, 0.0f, 100.0f);
  volume_ = volume;
  es8311_codec_set_voice_volume(static_cast<int>(volume_));
}

float Esp32P4Nano::volume() const { return volume_; }

void Esp32P4Nano::mute(bool mute) {
  mute_ = mute;
  es8311_set_voice_mute(mute_);
}

bool Esp32P4Nano::is_muted() const { return mute_; }

size_t Esp32P4Nano::play_audio(const uint8_t *data, uint32_t num_bytes) {
...
bool Esp32P4Nano::initialize_microphone(const microphone_callback_t &callback,
                                        const espp::Task::BaseConfig &task_config) {
  ...
  microphone_task_ = espp::Task::make_unique({
      .callback = std::bind(&Esp32P4Nano::microphone_task_callback, this, _1, _2, _3),
      .task_config = task_config,
  });
...
bool Esp32P4Nano::microphone_task_callback(std::mutex &m, std::condition_variable &cv,
                                           bool &task_notified) {
  size_t bytes_read = 0;
...
void Esp32P4Nano::microphone_volume(float volume) {
...
float Esp32P4Nano::microphone_volume() const { return mic_volume_; }

bool Esp32P4Nano::audio_task_callback(std::mutex &m, std::condition_variable &cv,
                                      bool &task_notified) {
