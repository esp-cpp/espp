...
public:
  ...
  /// the configured GPIO (CONFIG_ESP32_P4_NANO_TOUCH_INTERRUPT_GPIO).
#if CONFIG_ESP32_P4_NANO_TOUCH_INTERRUPT
  static constexpr gpio_num_t touch_interrupt_default =
      static_cast<gpio_num_t>(CONFIG_ESP32_P4_NANO_TOUCH_INTERRUPT_GPIO);
#else
  ...
  /// \note On the ESP32-P4-NANO the GT911 reset and INT pins are not routed to
  ...
  bool initialize_audio(uint32_t sample_rate = 48000,
                        const espp::Task::BaseConfig &task_config = {.name = "p4_nano_audio",
  ...
  bool initialize_camera(const camera_frame_callback_t &callback,
                         const espp::Task::BaseConfig &task_config = {.name = "p4_nano_camera",
...
protected:
  ...
  // The active panel is selected via Kconfig. On the ESP32-P4-NANO both panels
  ...
  // MIPI-DSI common parameters. The ESP32-P4-NANO wires 2 DSI data lanes and
  ...
  espp::Interrupt interrupts_{
      {.interrupts = {},
       .task_config = {.name = "p4-nano interrupts",
                       .stack_size_bytes = CONFIG_ESP32_P4_NANO_INTERRUPT_STACK_SIZE,
                       .priority = CONFIG_ESP32_P4_NANO_INTERRUPT_PRIORITY,
                       .core_id = CONFIG_ESP32_P4_NANO_INTERRUPT_CORE_ID}}};
