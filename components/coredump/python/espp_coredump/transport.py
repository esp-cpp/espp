"""USB vendor-interface transport for the core dump host tool.

Talks to the device's WebUSB / vendor interface (``bInterfaceClass == 0xFF``,
one bulk IN + one bulk OUT endpoint) — the same interface ``coredump_console.html``
...
class UsbVendorTransport:
    ...
    def _find_vendor_interface(self, cfg):
        ...
        raise TransportError(
            ...
            "is the device running the coredump example over its WebUSB/vendor interface?"
        )
