...
void Gui::init_draw_tab() {
  // instructions on the left, with the rotate / clear buttons on the right
  ...
  lv_obj_align(clear_button_, LV_ALIGN_TOP_RIGHT, 0, 60);
...
void Gui::init_imu_tab() {
  ...
  lv_label_set_text(imu_label_, "IMU initializing...");
  lv_obj_align(imu_label_, LV_ALIGN_TOP_LEFT, 0, 0);
  lv_obj_set_style_text_align(imu_label_, LV_TEXT_ALIGN_LEFT, 0);
  ...
  kalman_line_ = lv_line_create(imu_tab_);
  ...
  madgwick_line_ = lv_line_create(imu_tab_);
...
void Gui::init_audio_tab() {
  ...
  lv_obj_set_style_pad_row(audio_tab_, 10, 0);
  ...
  for (const auto &spec : buttons) {
    *spec.button = lv_btn_create(row);
    lv_obj_set_size(*spec.button, 44, 44);
...
void Gui::update_audio_label() {
  ...
  int speaker_volume = static_cast<int>(box.volume());
  int mic_volume = static_cast<int>(box.microphone_volume());
...
void Gui::on_pressed(lv_event_t *e) {
  ...
  if (target == volume_down_button_ || target == volume_up_button_) {
    ...
    box.volume(box.volume() + delta);
    logger_.info("Speaker volume: {:.0f}%", box.volume());
    update_audio_label();
    return;
  }
  if (target == mic_down_button_ || target == mic_up_button_) {
    float delta = target == mic_down_button_ ? -10.0f : 10.0f;
    box.microphone_volume(box.microphone_volume() + delta);
    logger_.info("Microphone volume: {:.0f}%", box.microphone_volume());
    update_audio_label();
    return;
  }
...
void Gui::set_imu_text(std::string_view text) {
  std::lock_guard<std::recursive_mutex> lock(mutex_);
  lv_label_set_text(imu_label_, std::string(text).c_str());
}
...
void Gui::set_down_line(lv_obj_t *line, lv_point_precise_t *points, float vx, float vy) {
  ...
  // draw the line from the center of the IMU tab's content area toward
  // "down" (fall back to the display size if the layout hasn't run yet)
  int w = lv_obj_get_content_width(imu_tab_);
  int h = lv_obj_get_content_height(imu_tab_);
  if (w <= 0 || h <= 0) {
