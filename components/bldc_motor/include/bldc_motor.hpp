  ...
  public:
    ...
    void set_phase_voltage(float uq, float ud, float el_angle) {
      ...
      case FocType::TRAPEZOID_120:
        ...
        sector = 6 * (normalize_angle(el_angle + _PI_6 ) / _2PI); // adding PI/6 to align with other modes
      ...
      case FocType::TRAPEZOID_150:
        ...
        sector = 12 * (normalize_angle(el_angle + _PI_6 ) / _2PI); // adding PI/6 to align with other modes
        // centering the voltages around either
        // modulation_centered_ == true > driver.volage_limit/2
        // modulation_centered_ == false > or Adaptable centering, all phases drawn to 0 when uq=0
        center = modulation_centered_ ? (driver_->get_voltage_limit())/2 : uq;
    ...
    void move(float new_target) {
      ...
      if (new_target) target_ = new_target;

      // calculate the back-emf voltage if KV_rating available U_bemf = vel*(1/KV)
      if (kv_rating_) bemf_voltage_ = shaft_velocity_ / kv_rating_ / RPM_TO_RADS;
      // estimate the motor current if phase reistance available and current_sense not available
      if (!current_sense_ && phase_resistance_) current_.q = (voltage_.q - bemf_voltage_) / phase_resistance_;
      ...
      case MotionControlType::ANGLE:
        ...
        target_current_ = pid_velocity_(target_shaft_velocity_ - shaft_velocity_); // if voltage torque control
        // if torque controlled through voltage
        if (torque_control_type_ == TorqueControlType::VOLTAGE) {
          // use voltage if phase-resistance not provided
          if (!phase_resistance_)  voltage_.q = target_current_;
          else voltage_.q =  std::clamp( target_current_ * phase_resistance_ + bemf_voltage_ ,
      ...
      case MotionControlType::VELOCITY:
        ...
        target_current_ = pid_velocity_(target_shaft_velocity_ - shaft_velocity_); // if current/foc_current torque control
      ...
      case MotionControlType::VELOCITY_OPENLOOP:
        ...
        target_shaft_velocity_ = target_;
        voltage_.q = velocity_openloop(target_shaft_velocity_); // returns the voltage that is set to the motor
        voltage_.d = 0;
        break;
      case MotionControlType::ANGLE_OPENLOOP:
        ...
        //                        version.
        target_shaft_angle_ = target_;
        voltage_.q = angle_openloop(target_shaft_angle_); // returns the voltage that is set to the motor
        voltage_.d = 0;
        break;
      }
    }
  ...
  protected:
    ...
    bool align_sensor() {
      ...
      if (sensor_direction_ == Direction::UNKNOWN) {
        ...
        if ( std::abs(moved*num_pole_pairs_ - _2PI) > 0.5f ) { // 0.5f is arbitrary number it can be lower or higher!
          logger_.debug("PP check: fail - estimated pp: {:.3f}", _2PI/moved);
        } else
          logger_.debug("PP check: OK!");

      } else logger_.debug("Skip dir calib.");
      ...
      if (!zero_electrical_angle_) {
        ...
        set_phase_voltage(0, 0, 0);
        std::this_thread::sleep_for(1ms * 200);
      } else logger_.debug("Skip offset calib.");
      return exit_flag;
    }
    ...
    float velocity_openloop(float target) {
      ...
      if (phase_resistance_) {
        Uq = std::clamp(current_limit_ * phase_resistance_ + std::abs(bemf_voltage_),
                        -voltage_limit_, voltage_limit_);
