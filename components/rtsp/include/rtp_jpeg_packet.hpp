...
namespace espp {
  ...
  public:
    ...
    explicit RtpJpegPacket(const int type_specific, const int frag_type,
                           const int q, const int width, const int height,
                           std::string_view q0, std::string_view q1,
  ...
  protected:
    ...
    static constexpr int PAYLOAD_OFFSET_WITH_QUANT = MJPEG_HEADER_SIZE + QUANT_HEADER_SIZE + (NUM_Q_TABLES * Q_TABLE_SIZE);
