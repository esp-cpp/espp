...
namespace espp {
  ...
  protected:
    ...
    bool send_response(int code, std::string_view message, int sequence_number=-1, std::string_view headers = "", std::string_view body = "") {
      // create a response
    ...
    bool handle_rtsp_describe(std::string_view request) {
      ...
      std::string body =
        "v=0\r\n" // version (0)
        "o=- " + std::to_string(session_id_) + " 1 IN IP4 " + server_address_ + "\r\n" // username (none), session id, version, network type (internet), address type, address
    ...
    bool handle_rtsp_setup(std::string_view request) {
      ...
      std::string headers =
        ...
        std::to_string(client_rtp_port) + "-" + std::to_string(client_rtcp_port) + "\r\n";
      return send_response(code, message, sequence_number, headers);
    }
    ...
    bool handle_rtsp_request(std::string_view request) {
      ...
      if (first_space_index == std::string::npos || second_space_index == std::string::npos || end_of_line_index == std::string::npos) {
        return handle_rtsp_invalid_request(request);
      }
      ...
      [[maybe_unused]] std::string_view rtsp_path = request.substr(first_space_index + 1, second_space_index - first_space_index - 1);
      // TODO: we should probably check that the rtsp version is correct
      [[maybe_unused]] std::string_view rtsp_version = request.substr(second_space_index + 1, end_of_line_index - second_space_index - 1);
    ...
    std::string_view parse_rtsp_path(std::string_view request) {
      // parse the rtsp path from the request
      // where the request looks like "METHOD RTSP_PATH RTSP_VERSION"
      std::string_view rtsp_path = request.substr(request.find(' ') + 1, request.find(' ', request.find(' ') + 1) - request.find(' ') - 1);
      return rtsp_path;
    }
    ...
    /// @return True if the request was parsed successfully, false otherwise
    bool parse_rtsp_setup_request(std::string_view request, std::string_view& rtsp_path, int& client_rtp_port, int& client_rtcp_port) {
      // parse the rtsp path from the request
      ...
      std::string_view transport = request.substr(transport_index + 11, transport_end_index - transport_index - 11);
      if (transport.empty()) {
        return false;
      }
      ...
      std::string_view rtp_port = request.substr(client_port_index + 12, dash_index - client_port_index - 12);
      if (rtp_port.empty()) {
        return false;
      }
      // parse the rtcp port from the request
      std::string_view rtcp_port = request.substr(dash_index + 1, request.find('\r', client_port_index) - dash_index - 1);
      if (rtcp_port.empty()) {
        return false;
      }
