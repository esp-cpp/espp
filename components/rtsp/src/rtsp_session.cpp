...
void RtspSession::handle_bad_rtsp_setup_request(std::string_view request, int code,
  ...
  send_response(code, message, sequence_number);
}
...
bool RtspSession::parse_rtsp_setup_request(std::string_view request, std::string_view &rtsp_path,
  ...
  if (rtsp_path.empty()) {
    ...
    handle_bad_rtsp_setup_request(request);
    return false;
  }
  ...
  if (transport.find("RTP/AVP/TCP") != std::string::npos) {
    ...
    handle_bad_rtsp_setup_request(request, 461, "Unsupported Transport");
    return false;
  }
