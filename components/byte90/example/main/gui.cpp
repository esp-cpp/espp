...
void Gui::init_background() {
  auto &byte90 = espp::Byte90::get();
  background_ = lv_obj_create(lv_screen_active());
  lv_obj_set_size(background_, byte90.rotated_display_width(), byte90.rotated_display_height());
...
void Gui::init_circle_layer() {
  ...
  lv_obj_set_size(circle_layer_, byte90.rotated_display_width(), byte90.rotated_display_height());
...
void Gui::next_rotation() {
  ...
  // update the size of the screen-filling objects and re-align the label
  lv_obj_set_size(background_, byte90.rotated_display_width(), byte90.rotated_display_height());
  lv_obj_align(label_, LV_ALIGN_CENTER, 0, 0);
  lv_obj_set_size(circle_layer_, byte90.rotated_display_width(), byte90.rotated_display_height());
  lv_obj_align(circle_layer_, LV_ALIGN_CENTER, 0, 0);
...
size_t Gui::visible_circle_count() {
  std::lock_guard<std::recursive_mutex> lock(mutex_);
