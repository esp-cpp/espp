...
static void IRAM_ATTR lcd_spi_pre_transfer_callback(spi_transaction_t *t) {
  static auto lcd_dc_io = Byte90::get_lcd_dc_gpio();
...
bool Byte90::initialize_lcd() {
  ...
  lcd_config_.mode = 0;
  lcd_config_.flags = SPI_DEVICE_NO_DUMMY | SPI_DEVICE_3WIRE;
  ...
  // lcd_config_.cs_ena_pretrans = 16;
  // lcd_config_.cs_ena_posttrans = 16;
  ...
  DisplayDriver::initialize(espp::display_drivers::Config{
      .write_command = std::bind(&Byte90::write_command, this, _1, _2, _3),
      .lcd_send_lines = std::bind(&Byte90::write_lcd_lines, this, _1, _2, _3, _4, _5, _6),
...
bool Byte90::initialize_display(size_t pixel_buffer_size) {
  ...
  display_ = std::make_shared<Display<Pixel>>(
      Display<Pixel>::LvglConfig{.width = lcd_width_,
                                 ...
                                 .rotation = rotation},
      Display<Pixel>::OledConfig{.set_brightness_callback = DisplayDriver::set_brightness,
...
void IRAM_ATTR Byte90::lcd_wait_lines() {
...
void IRAM_ATTR Byte90::write_command(uint8_t command, std::span<const uint8_t> parameters,
                                     uint32_t user_data) {
  lcd_wait_lines();
...
void IRAM_ATTR Byte90::write_lcd_lines(int xs, int ys, int xe, int ye, const uint8_t *data,
                                       uint32_t user_data) {
