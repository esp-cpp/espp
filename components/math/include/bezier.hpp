...
namespace espp {
  ...
  public:
    ...
    struct WeightedConfig {
      std::array<T, 4> control_points; ///< Array of 4 control points
      std::array<float, 4> weights = {1.0f, 1.0f, 1.0f, 1.0f}; ///< Array of 4 weights, default is array of 1.0f
    };
  ...
  protected:
    T eval(float t) const {
      ...
      auto mt3 = mt2*mt;
      return
        control_points_[0] * mt3 +
        control_points_[1] * 3.0f * mt2 * t +
        control_points_[2] * 3.0f * mt * t2 +
        control_points_[3] * t3;
    }
    ...
    T weighted_eval(float t) const {
      ...
      float f[] = {
        weights_[0] * mt3,
        weights_[1] * 3.0f * mt2 * t,
        weights_[2] * 3.0f * mt * t2,
        weights_[3] * t3
      };
      float basis = f[0] + f[1] + f[2] + f[3];
      return
        (f[0] * control_points_[0] +
         f[1] * control_points_[1] +
         f[2] * control_points_[2] +
