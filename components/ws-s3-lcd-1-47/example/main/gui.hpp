...
///   other tasks (button callbacks, the main loop, etc.) can safely call them.
///
/// For this example the Gui shows:
/// * A label in the center of the screen
/// * A custom-drawn layer of circles which the main loop draws in a ring
///   around the center of the screen
///
/// This board has no touchscreen, so there are no on-screen buttons; the
/// hardware button cycles the display rotation via next_rotation().
class Gui {
public:
  ...
  /// Get the maximum number of circles which can be visible at once.
  /// @return The maximum number of circles
  static constexpr size_t max_circles() { return MAX_CIRCLES; }

  /// Get the number of circles currently visible on the screen. Thread-safe.
...
protected:
  ...
  // resize / re-align the screen-filling objects for the current display
  // rotation
  void update_layout();
  ...
  espp::Task update_task_{
      {.callback = [this](auto &m, auto &cv) { return update(m, cv); },
       .task_config = {.name = "gui", .stack_size_bytes = 6 * 1024, .core_id = 1}}};
  espp::Logger logger_;
