...
static void IRAM_ATTR lcd_spi_pre_transfer_callback(spi_transaction_t *t) {
  static auto lcd_dc_io = WsS3Lcd147::get_lcd_dc_gpio();
...
bool WsS3Lcd147::initialize_lcd() {
  ...
  DisplayDriver::initialize(espp::display_drivers::Config{
      .write_command = std::bind(&WsS3Lcd147::write_command, this, _1, _2, _3),
      .lcd_send_lines = std::bind(&WsS3Lcd147::write_lcd_lines, this, _1, _2, _3, _4, _5, _6),
...
void WsS3Lcd147::write_command(uint8_t command, std::span<const uint8_t> parameters,
                               uint32_t user_data) {
  lcd_wait_lines();
