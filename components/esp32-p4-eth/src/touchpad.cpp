...
bool Esp32P4Eth::initialize_touch(const touch_callback_t &callback, gpio_num_t interrupt_pin) {
  if (touch_driver_) {
  ...
  touchpad_input_ = std::make_shared<TouchpadInput>(
      TouchpadInput::Config{.touchpad_read = std::bind_front(&Esp32P4Eth::touchpad_read, this),
  ...
  if (interrupt_pin != GPIO_NUM_NC) {
    ...
    // The touch INT pin is not wired to a GPIO on the ESP32-P4-ETH, so poll the
    ...
    touch_task_ = std::make_unique<espp::Task>(espp::Task::Config{
        ...
        .task_config = {.name = "p4-eth touch",
                        .stack_size_bytes = CONFIG_ESP32_P4_ETH_TOUCH_TASK_STACK_SIZE}});
    touch_task_->start();
  }
...
void Esp32P4Eth::touchpad_read(uint8_t *num_touch_points, uint16_t *x, uint16_t *y,
                               uint8_t *btn_state) {
  std::lock_guard<std::recursive_mutex> lock(touchpad_data_mutex_);
...
Esp32P4Eth::TouchpadData Esp32P4Eth::touchpad_convert(const TouchpadData &data) const {
