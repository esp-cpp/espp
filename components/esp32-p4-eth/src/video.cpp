...
bool Esp32P4Eth::initialize_lcd() {
  ...
  // channel 3 @ 2500 mV on the ESP32-P4-ETH).
  ...
  // NOTE: The ESP32-P4-ETH does not route a panel reset GPIO for either panel;
  ...
  if (!display_) {
    display_ = std::make_shared<Display<Pixel>>(
        Display<Pixel>::LvglConfig{.width = display_width_,
                                   ...
                                   .flush_callback = std::bind_front(&Esp32P4Eth::flush, this),
...
size_t Esp32P4Eth::rotated_display_width() const {
...
size_t Esp32P4Eth::rotated_display_height() const {
  auto rot = lv_display_get_rotation(lv_display_get_default());
  switch (rot) {
  case LV_DISPLAY_ROTATION_90:
  case LV_DISPLAY_ROTATION_270:
...
void Esp32P4Eth::write_lcd_lines(int xs, int ys, int xe, int ye, const uint8_t *data,
                                 uint32_t user_data) {
  (void)user_data;
...
void IRAM_ATTR Esp32P4Eth::flush(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map) {
  if (lcd_handles_.panel == nullptr) {
...
bool IRAM_ATTR Esp32P4Eth::notify_lvgl_flush_ready(esp_lcd_panel_handle_t panel,
                                                   esp_lcd_dpi_panel_event_data_t *edata,
                                                   void *user_ctx) {
  (void)panel;
  (void)edata;
  auto *board = static_cast<espp::Esp32P4Eth *>(user_ctx);
...
void Esp32P4Eth::dsi_write_command(uint8_t cmd, std::span<const uint8_t> params,
                                   uint32_t /*flags*/) {
  if (!lcd_handles_.io) {
...
void Esp32P4Eth::dsi_read_command(uint8_t cmd, std::span<uint8_t> data, uint32_t /*flags*/) {
  if (!lcd_handles_.io) {
