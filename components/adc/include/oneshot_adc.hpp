 ...
 *          you need both drivers on the P4, create the ContinuousAdc first,
 *          or keep the OneshotAdc alive.
 *
...
public:
   ...
   *         of \p channels; channels for a different unit are not configured.
   */
  struct Config {
   ...
   * @return std::optional<int> raw value for the provided channel config (if
   *         it was configured).
   * @note This function is provided for convenience; it is equivalent to \c
   *       read_raw(). This function is provided to match the API of \c
   *       ContinuousAdc, which has a \c get_raw() function.
   */
  std::optional<int> get_raw(const AdcConfig &config) { return read_raw(config); }

  /**
   * @brief Take a new ADC reading for the provided \p config and convert it to
   *        voltage (mV) if the channel's attenuation was properly calibrated.
   *        If it was not properly calibrated, then it will return the same
   *        value as \c read_raw().
   * @param config The channel configuration to take a reading from.
   * @return std::optional<int> Voltage in mV for the provided channel config
   *         (if it was configured).
   * @note This function is provided for convenience; it is equivalent to \c
   *       read_mv(). This function is provided to match the API of \c
   *       ContinuousAdc, which has a \c get_mv() function.
   */
  std::optional<int> get_mv(const AdcConfig &config) { return read_mv(config); }
   ...
   *        If it was not properly calibrated, then it will return the same
   *        value as \c read_raw().
   * @return std::vector<int> Voltage in mV for all configured channels (in
  ...
  std::vector<int> read_all_mv() {
    ...
    for (const auto &config : configs_) {
      ...
      if (err != ESP_OK) {
        ...
        values.push_back(raw_to_mv(raw, config.attenuation));
      }
    }
    return values;
  }
  ...
  std::optional<int> read_mv(const AdcConfig &config) {
    ...
    if (maybe_raw.has_value()) {
      return raw_to_mv(maybe_raw.value(), config.attenuation);
    }
    return {};
  }

protected:
  // adc_atten_t values are 0 (ADC_ATTEN_DB_0) through 3 (ADC_ATTEN_DB_12), so
  // we keep one calibration handle per attenuation (calibration curves are
  // specific to the (unit, attenuation) pair).
  static constexpr size_t NUM_ATTENUATIONS = 4;

  int raw_to_mv(int raw, adc_atten_t attenuation) {
    auto atten_index = static_cast<size_t>(attenuation);
    if (atten_index >= NUM_ATTENUATIONS || cali_handles_[atten_index] == nullptr) {
      logger_.warn_rate_limited(
          "attenuation {} not calibrated, cannot convert raw to mv - returning raw value!",
          attenuation);
      return raw;
    }
    int mv = raw;
    auto err = adc_cali_raw_to_voltage(cali_handles_[atten_index], raw, &mv);
    if (err != ESP_OK) {
  ...
  void init(const Config &config) {
    ...
    for (const auto &conf : config.channels) {
      ...
      if (config.unit != channel_unit) {
        logger_.error("{} invalid, main unit ({}) != channel unit ({}); not configuring it", conf,
      ...
      // create the calibration data for this channel's attenuation if we
      // don't have it yet
      auto atten_index = static_cast<size_t>(attenuation);
      if (atten_index < NUM_ATTENUATIONS && cali_handles_[atten_index] == nullptr) {
        calibration_init(config.unit, attenuation, &cali_handles_[atten_index]);
      }
    }
  }

  void calibration_init(adc_unit_t unit, adc_atten_t attenuation, adc_cali_handle_t *out_handle) {
