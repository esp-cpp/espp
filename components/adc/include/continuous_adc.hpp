...
namespace espp {
  ...
  public:
    ...
    struct Config {
      ...
      adc_digi_convert_mode_t convert_mode; /**< Conversion mode (unit 1, unit 2, alternating, or both). May depend on ESP32 chip. */
      size_t task_priority{5}; /**< Priority to run the adc data reading / filtering task. */
      size_t window_size_bytes{256}; /**< Amount of bytes to allocate for the DMA buffer when reading results. Larger values lead to more filtering. */
  ...
  protected:
    ...
    void update_task(std::mutex& task_m, std::condition_variable& task_cv) {
      ...
      float elapsed_seconds = std::chrono::duration<float>(current_timestamp - previous_timestamp).count();
    ...
    void init(const std::vector<AdcConfig>& channels) {
      ...
      if (conv_mode_ != ADC_CONV_SINGLE_UNIT_1) {
        logger_.error("Configured for ESP32, which only supports ADC_CONV_SINGLE_UNIT_1, "
                      "but provided invalid output convert mode {}", (int)conv_mode_);
      }
      ...
      output_format_ = (conv_mode_ == ADC_CONV_ALTER_UNIT || conv_mode_ == ADC_CONV_BOTH_UNIT) ?
        ADC_DIGI_OUTPUT_FORMAT_TYPE2 : ADC_DIGI_OUTPUT_FORMAT_TYPE1;
