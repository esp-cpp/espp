...
bool MatouchRotaryDisplay::initialize_lcd() {
  ...
  DisplayDriver::initialize(espp::display_drivers::Config{
      .lcd_write = std::bind(&MatouchRotaryDisplay::write_lcd, this, _1, _2, _3),
      .lcd_send_lines =
...
std::shared_ptr<espp::Display> MatouchRotaryDisplay::display() const { return display_; }
...
void IRAM_ATTR MatouchRotaryDisplay::write_lcd(const uint8_t *data, size_t length,
                                               uint32_t user_data) {
  if (length == 0) {
    return;
  }
...
uint16_t *MatouchRotaryDisplay::vram0() const {
  if (!display_) {
    return nullptr;
  }
...
uint16_t *MatouchRotaryDisplay::vram1() const {
  if (!display_) {
    return nullptr;
  }
