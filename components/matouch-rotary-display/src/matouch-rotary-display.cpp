...
void MatouchRotaryDisplay::lcd_wait_lines() {
  if (lcd_) {
    lcd_->wait();
  }
}
...
size_t MatouchRotaryDisplay::rotated_display_width() const {
...
size_t MatouchRotaryDisplay::rotated_display_height() const {
  auto *display = lv_display_get_default();
  auto rotation = display ? lv_display_get_rotation(display) : LV_DISPLAY_ROTATION_0;
  switch (rotation) {
  case LV_DISPLAY_ROTATION_90:
