...
static void lcd_spi_pre_transfer_callback(spi_transaction_t *t) {
  static auto lcd_dc_io = WsS3Touch::get_lcd_dc_gpio();
...
bool WsS3Touch::initialize_lcd() {
  ...
  DisplayDriver::initialize(espp::display_drivers::Config{
      .write_command = std::bind(&WsS3Touch::write_command, this, _1, _2, _3),
      .lcd_send_lines = std::bind(&WsS3Touch::write_lcd_lines, this, _1, _2, _3, _4, _5, _6),
...
bool WsS3Touch::initialize_display(size_t pixel_buffer_size) {
  if (!lcd_handle_) {
    logger_.error(
...
void WsS3Touch::write_command(uint8_t command, std::span<const uint8_t> parameters,
                              uint32_t user_data) {
  lcd_wait_lines();
...
void WsS3Touch::write_lcd_frame(const uint16_t xs, const uint16_t ys, const uint16_t width,
...
WsS3Touch::Pixel *WsS3Touch::vram1() const {
  if (!display_) {
    return nullptr;
  }
