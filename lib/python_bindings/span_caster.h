...
template <size_t N> struct span_name_maker {
  template <typename T> static constexpr auto make(const T &t) {
    return concat(t, span_name_maker<N - 1>::make(t));
  }
};

template <> struct span_name_maker<1> {
  template <typename T> static constexpr auto make(const T &t) { return t; }
};
...
template <typename Type, size_t Extent> struct type_caster<std::span<Type, Extent>> {
  ...
  PYBIND11_TYPE_CASTER(span_type,
                       _("Tuple[") + span_name_maker<Extent>::make(value_conv::name) + _("]"));
...
public:
  template <typename T> static handle cast(T &&src, return_value_policy policy, handle parent) {
    ...
    for (auto &&value : src) {
      auto value_ =
          reinterpret_steal<object>(value_conv::cast(forward_like<T>(value), policy, parent));
      if (!value_)
        return handle();
